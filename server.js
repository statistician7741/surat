...
let runServer = () => {
  ...
  app.prepare().then(() => {
    ...
    server.use(
      cookieParser('ID==&&%^&A&SHBJSAsjhbJGhUGkbKiUvii^%^#$%^&98G8UIugg==')
    );
    server.use(
    ...
    server.use(
      '/arsip/suratmasuk/download',
      express.static(`${__dirname}/arsip/suratmasuk`)
    );
    ...
    serve.listen(port, (err) => {
      if (err) throw err;
      console.log(`> Ready on ${config.SISUKMA_HOST}`);
    });
...
let start = () => {
  mongoose.connect(
    ...
    (err) => {
      if (err) {
        exec(
          `powershell -Command "Start-Process cmd -Verb RunAs -ArgumentList '/c net start MongoDB'"`,
          (err, stdout, stderr) => {
